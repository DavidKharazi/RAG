...
os.environ['OPENAI_API_KEY'] = 'sk-proj-S95MArdKEFLrkHb13OMwT3BlbkFJHHshKMPnBbPGgroLC7yr'
...
API_URL = "http://0.0.0.0:8222/rag_chat/"

async def ask_question(question):
    async with aiohttp.ClientSession() as session:
        async with session.post(API_URL, json={'question': question}) as response:
...
async def send_welcome(message: types.Message):
    await message.reply("Hi! I'm your assistant bot. Ask me any job-specific questions!")

@dp.message_handler()
async def handle_question(message: types.Message):
    answer = await ask_question(message.text)
...
if __name__ == "__main__":
    uvicorn.run('RAG1:app', host="0.0.0.0", port=8222)